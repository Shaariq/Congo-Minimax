...
int calculateAttackScore(vector<string> moves, array<array<string, 7>, 7> board, string turn)
{
    int score = 0;
    ...
    for (int i = 0; i < moves.size(); i++)
    {
        ...
        // get token of piece moving to
        string pieceToken = board[endingRow][endingCol];

        if (turn == "white")
        {
            // if it is black and a lion add 10, else add 1
            if (isBlack(pieceToken))
            {
                score += 1;
        ...
        {
            // if it is white and a lion add 10, else add 1
            if (isWhite(pieceToken))
            {
                score += 1;
...
int evaluation(array<array<string, 7>, 7> board, string turn)
{
    ...
    {
        materialScore = whitePieces - blackPieces;
        
        vector<string> whiteMoves = getMoves(board, "white");
        vector<string> blackMoves = getMoves(board, "black");

        mobilityScore = whiteMoves.size() - blackMoves.size();

        attackScore = calculateAttackScore(whiteMoves, board, "white") - calculateAttackScore(blackMoves, board, "black");
...
int alphaBeta(array<array<string, 7>, 7> board, int depth, string turn, int alpha, int beta)
{
    ...
    for (int i = 0; i < moves.size(); i++)
    {
        ...
        int eval = -alphaBeta(nextState, depth - 1, newTurn, -beta, -alpha);
...
int main()
{
    ...
    for (int i = 0; i < fenStrings.size(); i++)
    {
        ...
        int score = alphaBeta(board, 4, turnToPlay, 99999999*-1, 99999999);
        cout << score << endl;
    }
